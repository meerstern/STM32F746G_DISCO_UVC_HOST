...
uint8_t  USBH_FindInterface(USBH_HandleTypeDef *phost, uint8_t Class, uint8_t SubClass, uint8_t Protocol)
{
  ...
  while (if_ix < USBH_MAX_NUM_INTERFACES)
  {
    ...
    printf("Index:%d, Class:0x%X, SubClass:%d, Prtcl:%d\n\r",if_ix,pif->bInterfaceClass,pif->bInterfaceSubClass,pif->bInterfaceProtocol);

    printf("0Type:0x%x, Addr:0x%x, Inval:0x%x, Len:0x%x, Attr:0x%x, Max:0x%x\n\r",
...
//    printf("1Type:0x%x, Addr:0x%x, Inval:0x%x, Len:0x%x, Attr:0x%x, Max:0x%x\n\r",
...
USBH_StatusTypeDef  USBH_Process(USBH_HandleTypeDef *phost)
{
  ...
  switch (phost->gState)
  {
    ...
    case HOST_DEV_ATTACHED :
      ...
      printf("Pipe In:0x%x, Out:0x%x\n\r",phost->Control.pipe_in,phost->Control.pipe_out);
...
static USBH_StatusTypeDef USBH_HandleEnum(USBH_HandleTypeDef *phost)
{
  ...
  switch (phost->EnumState)
  {
    ...
    case ENUM_GET_FULL_CFG_DESC:
      ...
      //printf("ENUM_GET_FULL_CFG_DESC:%d\n\r",ReqStatus);
      if (ReqStatus == USBH_OK)
      {
